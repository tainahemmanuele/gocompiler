...
public class GoSemanticSequencer extends AbstractDelegatingSemanticSequencer {
			...
			case GoPackage.PARAMETERS:
				if (rule == grammarAccess.getConstSpecRule()) {
					...
					return; 
						...
						|| rule == grammarAccess.getMethodDeclRule()) {
					sequence_MethodDecl_Parameters(context, (Parameters) semanticObject); 
					return; 
				}
	 ...
	 *
	 * Constraint:
	 *     (signature+=Signature body+=FunctionBody?)
	 */
	...
	/**
	 * Contexts:
	 *     TopLevelDecl returns Parameters
	 *     MethodDecl returns Parameters
	 *
	 * Constraint:
	 *     (parameterlist=ParameterList? signature+=Signature body+=FunctionBody?)
	 */
	protected void sequence_MethodDecl_Parameters(ISerializationContext context, Parameters semanticObject) {
		genericSequencer.createSequence(context, semanticObject);
	}
	...
	protected void sequence_PackageClause(ISerializationContext context, PackageClause semanticObject) {
		if (errorAcceptor != null) {
			if (transientValues.isValueTransient(semanticObject, GoPackage.Literals.PACKAGE_CLAUSE__PACK) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, GoPackage.Literals.PACKAGE_CLAUSE__PACK));
		}
		SequenceFeeder feeder = createSequencerFeeder(context, semanticObject);
		feeder.accept(grammarAccess.getPackageClauseAccess().getPackPackageNameParserRuleCall_1_0(), semanticObject.getPack());
		feeder.finish();
	}
